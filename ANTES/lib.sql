...
-- NAO OFICIAL??
--DROP FUNCTION lib.kxrefresh_quadrasc(float,int,boolean,boolean,boolean,boolean,boolean);
CREATE OR REPLACE FUNCTION lib.kxrefresh_quadrasc(
    -- Ver documentação em
    -- http://gauss.serveftp.com/colabore/index.php/Prj:Geoprocessing_Recipes/R009
    --
    ...
    p_out_errsig   BOOLEAN DEFAULT true,   -- true se for para, junto com kx.lote_viz, gerar erros em errsig 
    p_drop_lote_viz BOOLEAN DEFAULT false, -- true se for para apagar kx.lote_viz depois de usada.
    p_listarvias BOOLEAN DEFAULT false  -- false se for para não gerar lista de vias vizinhas.
) RETURNS text AS $func$
   DECLARE
     aux int;
     ret text;
   ...
   ret:=ret||E' -- ... Rodando kxrefresh_quadrasc ...';
   ...
   IF p_gerapol THEN 
     ...
     CREATE TABLE kx.quadrasc AS
       SELECT kx_quadrasc_id AS gid, 
              ...
              array_distinct(array_agg(kx_quadra_gid)) AS quadra_gids,
